"""
Cinematic Photorealistic Animated Avatar Engine for MARK-54 JARVIS HUD.

Renders an ultra-high-definition photorealistic human avatar with:
  - Natural biological eyelid blinking with skin-crease curvature & lash lines
  - Real-time viseme-synchronized mouth articulation, inner depth, and teeth
  - Organic head breathing sway and gaze micro-saccades
  - Glowing holographic segmented neon HUD rings with rotating ticks
  - Symmetrical real-time audio equalizer spectrum
  - Futuristic state visualizer (LISTENING / THINKING / SPEAKING)
"""
...
class RealisticAvatar:
    """Cinematic photorealistic animated avatar with real-time lip sync and HUD rings."""
    ...
    def step(
        ...
        if now >= self._blink_next:
            ...
            self._blink_next = now + random.uniform(2.8, 5.0)
        ...
        if now >= self._gaze_next:
            if "THINK" in self._state:
                ...
                self._gaze_tx = random.uniform(-0.12, 0.12)
                self._gaze_ty = random.uniform(-0.08, 0.08)
        ...
        # ── 4. Sway & Breathing ──
        self._sway_x = math.sin(self._t * 0.8) * 2.2 + (self._gaze_x * 2.8)
        self._sway_y = math.sin(self._t * 1.4) * 2.8 + (0.4 if speaking else 0.0)
        self._sway_rot = math.sin(self._t * 0.6) * 0.7
    ...
    def paint(
        ...
        """Render the cinematic photorealistic avatar, HUD rings, and equalizer."""
        p.save()
        ...
        glow_col = QColor(primary.red(), primary.green(), primary.blue(), min(200, glow_alpha))
        ...
        # ── 3. Draw Photorealistic Head & Features ──
        head_w = r * 1.95
        ...
        if not self._pixmap.isNull():
            ...
            # Real-time Eyelid Blinking
            self._draw_eyelids(p, head_rect)

            # Real-time Lip-Sync Talking
