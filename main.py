...
class JarvisLive:
    ...
    def _on_text_command(self, text: str):
        if not self._loop or not self.session:
            return
        # A typed command is deliberate control: wake JARVIS if asleep
        if self._wake_enabled and not self._awake:
            self.wake(reason="text command")
    ...
    async def _play_audio(self):
        ...
        # Dual output: also pipe audio to VB-Audio Cable Input if present (for WhatsApp call mic)
        cable_stream = None
        try:
            for _idx, _d in enumerate(sd.query_devices()):
                if _d.get('max_output_channels', 0) > 0 and 'cable input' in _d.get('name', '').lower():
                    cable_stream = _open_spk(_idx)
                    print(f"[JARVIS] 🎙️ Virtual Cable audio mirroring active on device {_idx} ({_d['name']})")
                    break
        except Exception as _ce:
            print(f"[JARVIS] Virtual Cable output not opened: {_ce}")
            ...
            while True:
                try:
                    ...
                    if (
                        ...
                        self.set_speaking(False)
                        self._turn_done_event.clear()
                        # Auto-hangup trigger for WhatsApp receptionist
                        try:
                            from actions.whatsapp_receptionist import is_auto_hangup_armed, trigger_hangup_now
                            if is_auto_hangup_armed():
                                await asyncio.sleep(0.35)  # Ensure final audio chunk flushed through DAC/Cable
                                trigger_hangup_now(player=self.ui)
                        except Exception as _he:
                            print(f"[Receptionist] Auto-hangup trigger error: {_he}")
                    continue
                ...
                def _write_dual(data_bytes):
                    try:
                        stream.write(data_bytes)
                    except Exception:
                        pass
                    if cable_stream is not None:
                        try:
                            cable_stream.write(data_bytes)
                        except Exception:
                            pass

                try:
                    await asyncio.to_thread(_write_dual, bytes(batch))
    ...
    # ── WhatsApp / Phone Call Proactive Watcher ────────────────────────────────
    async def _run_whatsapp_call_watcher(self) -> None:
        """
        Background task: continuously monitors Windows notifications for incoming
        WhatsApp or phone calls. When a new incoming call is detected:
        1. Wakes JARVIS if asleep.
        2. Logs the incoming call on the HUD.
        3. Proactively prompts Gemini Live to announce the call over voice and ask Sir
           if JARVIS should answer as an AI receptionist or answer/decline the call.
        """
        from actions.whatsapp_receptionist import check_incoming_call_async, is_call_in_progress
        ...
        while True:
            await asyncio.sleep(1.0)
            try:
                # If a call is already answered / in progress, do not poll or interrupt
                if is_call_in_progress():
                    await asyncio.sleep(2.0)
                    continue

                # If JARVIS is currently speaking, wait
                with self._speaking_lock:
                    speaking = self._is_speaking
                if speaking:
                    continue

                call_info = await check_incoming_call_async()
                if not call_info:
                    continue

                call_id = call_info.get("id")
                caller = call_info.get("caller") or "WhatsApp Caller"
                source = call_info.get("source") or "WhatsApp"

                now = time.time()
                # Cooldown between repeated announcements of the same call ID
                if call_id == last_announced_id and (now - last_announced_time) < 45.0:
                    continue

                last_announced_id = call_id
                last_announced_time = now

                # Display incoming call alert on HUD
                self.ui.write_log(f"SYS: Incoming {source} call from {caller}.")
                if hasattr(self.ui, "show_content"):
                    self.ui.show_content(
                        f"INCOMING {source.upper()} CALL",
                        f"Caller: {caller}\n\nStatus: Ringing...\nSay 'tell him I am busy' to auto-reply & cut, or 'answer the call'."
                    )

                # Ensure JARVIS is awake
                if self._wake_enabled and not self._awake:
                    self.wake(reason=f"incoming {source} call")

                # Wait for session if reconnecting
                if not self.session:
                    continue

                prompt = (
                    f"[INCOMING_{source.upper()}_CALL] There is an incoming {source} call right now from {caller}. "
                    f"Immediately ask Sir in one short sentence: 'Sir, {caller} is calling you on {source}. Should I answer the call?'"
                )
                await self.session.send_client_content(
                    turns=[{"role": "user", "parts": [{"text": prompt}]}],
                    turn_complete=True,
                )
                print(f"[JARVIS] Proactively announced incoming call from {caller}.")
            except Exception as e:
                await asyncio.sleep(1.0)
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    tg.create_task(self._run_whatsapp_call_watcher())
                    if self._dashboard:
