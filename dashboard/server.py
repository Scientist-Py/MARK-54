...
<p>Connecting to JARVIS…</p>
        ...
        async def compose_page():
            compose_file = STATIC_DIR / "compose.html"
            if compose_file.exists():
                return HTMLResponse(compose_file.read_text(encoding="utf-8"))
            return HTMLResponse("<h3>Gmail Composer not found in static dir.</h3>")

        @app.get("/api/drafts/latest")
        async def get_latest_draft():
            return JSONResponse({"ok": True, "draft": self._latest_draft})

        @app.post("/api/drafts")
        async def set_draft_ep(req: Request):
            try:
                body = await req.json()
                self._latest_draft = body
                return JSONResponse({"ok": True})
            except Exception as e:
        ...
        async def send_email_api(req: Request):
            try:
                ...
                if not to or not body:
                    return JSONResponse({"ok": False, "error": "Recipient and body are required."}, status_code=400)

                from actions.gmail_helper import _get_gmail_service, _send_email_direct
                service = _get_gmail_service()
                if not service:
                    return JSONResponse({"ok": False, "error": "Gmail is not authenticated."}, status_code=401)

                res = _send_email_direct(service, to, subject, body)
                asyncio.create_task(self.broadcast({
                    "type": "sys",
                    "text": f"Email successfully sent to {to} via Gmail."
                }))
        ...
        async def save_text_api(req: Request):
            try:
                data = await req.json()
                title = (data.get("title") or "Piece").strip()
                content = (data.get("content") or "").strip()
                category = (data.get("category") or "text").strip().lower()

                if not content:
                    return JSONResponse({"ok": False, "error": "No content provided."}, status_code=400)

                from actions.text_writer import _get_desktop_dir
                import re
                ...
                asyncio.create_task(self.broadcast({
                    "type": "sys",
                    "text": f"Saved {category} '{title}' to Desktop as {filename}."
                }))
                return JSONResponse({"ok": True, "filename": filename, "path": str(target_path)})
            except Exception as e:
                return JSONResponse({"ok": False, "error": str(e)}, status_code=500)
